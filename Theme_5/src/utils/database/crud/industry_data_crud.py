...
class IndustryDataCRUD:
    @staticmethod
    @db_catcher("Creating industry aggregated data")
    async def create_industry_aggregated_date(session: AsyncSession, region_data: dict):
        new_region = RegionDataORM(
            region=region_data['industry'],
    ...
    @staticmethod
    @db_catcher("Truncating industry_data table")
    async def truncate_industry_data(session: AsyncSession):
        await session.execute(text('TRUNCATE TABLE industry_data RESTART IDENTITY CASCADE'))

    @staticmethod
    async def calculate_industry_aggregate_data(session: AsyncSession, df: pd.DataFrame):
        await IndustryDataCRUD.truncate_industry_data(session)
        region_agg = df.groupby('industry').agg({
            'business_value': 'mean',
        ...
        for _, row in region_agg.iterrows():
            await IndustryDataCRUD.create_industry_aggregated_date(session, row.to_dict())

        await session.commit()

        await CommonInfoIndustryCRUD.calculate_common_industry_info(session, df)
